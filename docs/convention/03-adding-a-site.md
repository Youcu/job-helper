...
- [ ] **직무는 `.env` 에 안 적는다.** 사람은 `JOB_ROLES=백엔드,웹` 이라고 사이트 중립
      이름으로 한 번만 적고, 사이트는 `tags/<site>_role_map.json` 으로 자기 코드를 찾는다
      (`_common/roles.resolve`). 사이트마다 항목을 두면 여섯 벌을 맞춰야 한다
- [ ] **한 이름이 여러 코드로 펼쳐질 수 있다.** 사이트가 직무를 쪼갠 방식이 달라 1:1 이
      아니다 — Wanted 는 `백엔드` 가 코드 여섯이다 (언어별 직무가 따로 있다)
- [ ] **대응 코드가 없는 역할은 조용히 빼지 말고 알려라.** `missing_roles` 로 돌려받아
      실행 화면에 찍는다. 조용히 빠지면 왜 결과가 적은지 못 찾는다
