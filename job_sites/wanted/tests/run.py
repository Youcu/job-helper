...
먼저 찔러 본 뒤, 실제로 틀린 것만 굳혔다. 처음 돌렸을 때 9건이 실패했고 전부 실제 결함이었다.
그 테스트에는 `# 결함:` 주석이 붙어 있다.

네트워크를 타지 않는다 — `helpers.FakeClient`·`FakeSession` 과 떠 놓은 응답
`fixtures/job_details.json` 을 쓴다.
"""
